...
CreateTableOne2 <- function(cluster,
  ...
  table1 <- CreateTableOne(vars = cluster,
                           strata = strata,
                           data = data1)
  table1 <- print(table1,showAllLevels = showAllLevels, printToggle = FALSE,noSpaces = TRUE)
  ...
  ## 保存文件
  if(save.file == T){
    write.csv(table1,paste0(names,"_baseline information.csv")) # write_xlsx should not be used
  }
